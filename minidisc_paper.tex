...
\title{Minidiscs in Binary Black Hole Accretion}
...
Newtonian simulations have demonstrated that accretion onto binary black holes is mediated by accretion discs around each black hole (``minidisks''), fed by gas streams flowing through the circumbinary cavity from the surrounding circumbinary disc. We study the dynamics and radiation of a black hole minidisc using two-dimensional hydrodynamical simulations performed with a new general relativistic version of the moving mesh code Disco.  We find that tidal forces from the binary companion and the impact of inflowing accretion streams excite spiral shock waves in the minidisc which effectively dissipate energy and transport angular momentum outwards inducing efficient accretion onto the black hole. We implement a local blackbody cooling presciption and find that the discs evolve to a selfconsistant temperature profile. The shock structure is found to be in agreement with the relativistic dispersion relation for tightly-wound linear waves. We measure the shock induced dissipation and find effective alpha parameters on the order of 0.01. We present spectra and viewing angle dependant images of the minidiscs from the blackbody radiation model.
...
Gas accretion onto binary black holes influences their orbital evolution and can cause electromagnetic emission associated with gravitational wave emission.

During hierarchical structure formation galactic mergers 
...
The density in the nozzle is set by the accretion rate $\dot{M}$, the main parameter of interest in this study.  The inclusion of dimensionful $\ka$ and $\sig_{SB}$ parameters in the cooling term breaks the scale invariance of mass-energy that would otherwise be present.  Streams of different $\dot{M}$ will cool at different rates relative to the orbital period $T_{bin}$, leading to hotter or cooler discs.  The density in the stream has a profile $\Sig \propto \cos^2(\pi \phi / \De \phi)$, with the normalization set to ensure a total accretion rate of specified $\dot{M}$.
...
Although the inner boundary is outside the event horizon, we find it does not affect the evolution of the system.  This is because it is still inside the sonic radius of the flow, so no information can propagate out to the minidisc itself.  The CFL limited time step $\De t$ is controlled by the innermost zones of the grid, where the radial velocity is large (due to the black hole) and the cells are narrow.  Placing the inner boundary above the event horizon allows for much larger time steps than otherwise possible, keeping it below the sonic point ensures the fidelity of the calculation.  We found $r_{in} = 4M$ to be a happy medium.
...
The relative strength of an accretion flow is typically measured by determining an effective Shakura-Sunyaev (or Novikov-Thorne) $\al$-parameter.  The gravitational forces of the companion, plus the global character of the shocks providing the dissipation, make $\al_{eff}$ a somewhat fuzzy notion.  We present two measures of $\al_{eff}$.  First, following \cite{Ju16}, we calculate the $\al$ required at each radii for a Novikov-Thorne disc of the same average temperature and accretion rate.  Second, \cite{Rafikov16} determines for shocked Newtonian discs $\al_{eff} \sim \psi_Q / 3\pi$.  We take this same prescription without modification, as $\psi_Q$ is a scalar quantity and hence frame independent.  
