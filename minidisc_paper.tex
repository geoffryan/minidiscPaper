...
A circular binary emits gravitational waves which carry away energy and angular momentum and eventually lead to merger.  The time for a circular binary to merge \citep{Peters64}, in units of the initial orbital period, is:
\begin{equation}
	T_{merge} / T_{bin} = \frac{5}{512 \pi}\frac{(1+q)^2}{q} \left(\frac{a}{M_{bin}}\right)^{5/2} \ . \label{eq:Tmerge}
\end{equation}
For the system we consider $a=100 M_{bin}$ and $q=0.1$, so $T_{merger} \approx 3.7\times 10^3 T_{bin}$.  The simulations run for $\approx 30 T_{bin}$, so we do not include evolution of the binary orbital parameters at this time.
