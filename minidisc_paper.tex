...
\model{2-hr} & $2\times10^{-3}$ & ? & $4M$ & $0.11$ & $512$ & $28$ & $29$ \\
...
As seen in Fig \ref{fi:diss} the rate of cooling in the inner disc $r < 10M$ is in significant excess to the standard Novikov-Thorne model.  This naturally leads to the question as to whether there is an observational difference between a standard thin disc and one undergoing shock-driven accretion.  We calculate the observational spectrum of each model using the ray tracing methods of Sec \ref{subsec:raySpec}, based on the method of \cite{Kulkarni11}.
...
An image of the specific intensity $I_{0.1keV}$ for \model{2} at $t=28 T_{bin}$ viewed at inclination $i=60^\circ$ is shown in Fig \ref{fi:im}.  The total emission is dominated by the bright inner region, and the two-armed structure is clearly visible. We generate the spectrum by integrating over such images at several frequencies.  The spectrum corresponding to Fig. \ref{fi:im} is shown in Fig. \ref{fi:q011_m3_spec}. 

The spectral differences between \model{2} and a Novikov-Thorne spectrum are subtle.  This is due to the insensitive nature of $\dot{Q}_{cool}$ in thin discs, see the discussion in \ref{subsec:diss}.

{\color{red} GEOFF to ANDREW: 

\begin{figure*}
	\plotone{figs/q011_m3_spec.png}
	\caption{\label{fi:q011_m3_spec} Spectrum of \model{2} at $t = 28 T_{bin}$ (black crosses), obtained by integrating over the ray-traced intensity in Fig \ref{fi:im}.  Novikov-Thorne spectrum for a disc at the same distance with $\dot{M}$ of the inner boundary averaged over $20 T_{bin}$ to $29 T_{bin}$.}
\end{figure*}
...
To verify our boundary condition does not affect the global evolution, we mapped the \model{2} solution at $t=25 T_{bin}$ onto a new grid that does extend within the horizon.  This grid has $r_{min} = 1.8M$, and is referred to as \model{2-bc}.  We evolve \model{2-bc} for a single $T_{bin}$, and see no differences compared with \model{2} at the same time.  

{\color{red}GEOFF to ANDREW: Not sure which plot to show here.}
