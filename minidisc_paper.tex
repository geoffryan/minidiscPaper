...
We compare the predicted torque due to spiral shocks eq \eqref{eq:tauRaf} to the measured $\tau_{Re}$ and $\tau_{\dot{M}}$ for Model 2 in Fig \ref{fi:torque-fid}.  The measured torques are calculated by azimuthally integrating grid quantities and centred differencing in $r$ when necessary.  The predicted torques due to irreversible shock heating, on the other hand, are calculated directly from the entropy jump $\De s$ measured to calculate $\psi_Q$.  The shock torque model $\tau_{irr}$ shows good agreement with $\tau_{Re}$, deviating by at most $30\%$ in $r < 60M$.  Since the disc is not in a steady state, one would not expect the accretion torque $\tau_{\dot{M}}$ to match exactly with the other torques, although it is evident they follow a similar profile.

\begin{figure}
	\plotone{figs/q011_m3_torque_r.pdf}
	\caption{\label{fi:torque-fid} Time averaged accretion torque $\tau_{\dot{M}}$ (black crosses), Reynolds torque $\tau_{Re}$ (blue crosses), predicted torque from spiral shocks $\tau_{irr}$ (eq. \eqref{eq:tauRaf}, red crosses), and predicted torque from local dissipation $\tau_t$ (eq. \eqref{eq:tauLoc}, orange crosses) for the fiducial run. Time averaging was over $(20-29)T_{bin}$.}  
\end{figure}
